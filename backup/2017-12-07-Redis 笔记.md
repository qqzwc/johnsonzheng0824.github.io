...
![](/images/posts/redis/Snipaste_2018-05-17_22-27-25.png)

说明

配置位置：
指定是否在每次更新操作后进行日志记录，Redis在默认情况下是异步的把数据写入磁盘，如果不开启，可能会在断电时导致一段时间内的数据丢失。因为 redis本身同步数据文件是按上面save条件来同步的，所以有的数据会在一段时间内只存在于内存中。默认为no<br>
appendonly no

**总结**

![](/images/posts/redis/Snipaste_2018-05-17_23-23-47.png)
...
因为RDB文件只用作后备用途，建议只在Slave上持久化RDB文件，而且只要15分钟备份一次就够了，只保留save 900 1这条规则。<br>
如果Enable AOF，好处是在最恶劣情况下也只会丢失不超过两秒数据，启动脚本较简单只load自己的AOF文件就可以了。代价一是带来了持续的IO，二是AOF rewrite的最后将rewrite过程中产生的新数据写到新文件造成的阻塞几乎是不可避免的。只要硬盘许可，应该尽量减少AOF rewrite的频率，AOF重写的基础大小默认值64M太小了，可以设到5G以上。默认超过原大小100%大小时重写可以改到适当的数值。<br>
...
> 先 watch 再开启 multi，一旦执行了 exec 之前加的监控锁都会被取消掉
...
![](/images/posts/redis/Snipaste_2018-05-18_19-22-14.png)
...
2. 从机是否可以写？set可否？<br>
答：否
3. 主机shutdown后情况如何？从机是上位还是原地待命<br>
答：原地待命
4. 主机又回来了后，主机新增记录，从机还能否顺利复制？<br>
答：能
5. 其中一台从机down后情况如何？依照原有它能跟上大部队吗？<br>
答：不能，要重新连接
...
![](/images/posts/redis/Snipaste_2018-05-18_21-39-31.png)
...
## Java 中的 Redis 操作（Jedis）和 Spring 整合 Redis

**参考 [GitHub 项目](https://github.com/johnsonzheng0824/RedisTest)**
